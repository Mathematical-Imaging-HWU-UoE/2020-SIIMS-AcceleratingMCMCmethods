...
Lg = alpha; %%% Lipshcitz constant of the prior
...
gradG = @(x) alpha*x; %%% gradient of the prior
gradU = @(x) gradF(x) + gradG(x); %%% gradient of the model
logPi = @(x) -(norm(y-A(x),'fro')^2)/(2*sigma2) -0.5*alpha*norm(x,2)^2;
...
ex = ifft2((HC_FFT * H_FFT + 0.07 * eye(256)) \ fft2(AT(y)));


%ex = ifft2(inv(HC_FFT * H_FFT + 0.07 .* (eye(256))) * fft2(ATy));
imshow(real(ex))
% 
% if 0
% 
